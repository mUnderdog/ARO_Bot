...
def run_pipeline(query, num_results, researcher_prompt, email_prompt):
    ...
    if not GROQ_API_KEY:
        log("GROQ_API_KEY not found in .env — LLM features will fail.", "err")
...
with st.sidebar:
    ...
    # ── Env status ──
    from config import GROQ_API_KEY, GOOGLE_API_KEY, GOOGLE_CSE_ID
    ...
    with col_a:
        st.markdown(
            ...
            f'background:{"rgba(63,185,80,.1)" if groq_ok else "rgba(248,81,73,.1)"};'
            f'border:1px solid {"rgba(63,185,80,.3)" if groq_ok else "rgba(248,81,73,.3)"};'
            f'color:{"#3fb950" if groq_ok else "#f85149"};text-align:center;">'
            f'{"✔" if groq_ok else "✖"} Groq</div>', unsafe_allow_html=True)
    with col_b:
        st.markdown(
            f'<div style="font-size:.75rem;padding:.35rem .6rem;border-radius:6px;'
    ...
    if not groq_ok:
        st.warning("Add GROQ_API_KEY to your `.env` file.", icon="⚠️")
    ...
    st.markdown('<div class="sf">ARO Bot v1.0 · Groq (Llama 3) · For freshers 🎓</div>',
                unsafe_allow_html=True)
...
if run_clicked:
    if not _import_ok:
        ...
        st.error("GROQ_API_KEY is missing from .env — cannot run.")
    else:
...
<div class="hero">
    ...
    <p>Discover AI/tech companies, auto-research them with Groq, and generate
    ...
    <div class="chips">
        ...
        <div class="chip"><span class="dot" style="background:#a78bfa"></span>Groq (Llama 3)</div>
